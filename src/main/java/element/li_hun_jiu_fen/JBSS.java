package element.li_hun_jiu_fen;
...
import template.util.TimePro;

/**
 * description:
 * Created by gaoyw on 2019/1/7.
 */
public class JBSS {
     ...
     */
    public static void XSSJandRHXS(Document document,Element newroot){
        ...
        String rhxs = "未提及";
        ArrayList xsdateList = new ArrayList();
        if(root.attribute("value")!=null){
            ...
            for(String qw:qwStrarray){
                ...
                if(rs){
                    if(qw.contains("相识")){
                        TimePro.getDate(qw, xsdateList, JBSS.datePattern);
                        break;
                    }
        ...
        if(xsdateList.size()>0){
            xsdate = (String) xsdateList.get(0);
        }
     ...
     * 此方法把相识字符串里的所有时间都提取出来了，目前没有两个日期
     */
    public static  void QLLASJ(Document document,Element newroot){
        Element root = document.getRootElement();
        Element QDLAGXSJNode = newroot.addElement("QDLAGXSJ").addAttribute("nameCN", "确定恋爱关系时间");

        Pattern pattern = Pattern.compile(JBSS.datePattern);
        String qdlagxsj = "未提及";
        ArrayList qllagxsj = new ArrayList();
        if(root.attribute("value")!=null){
            String qwStr = root.attributeValue("value");
            String[] qwStrarray = qwStr.split("。|，|；");
            //建立恋爱时间
            for(String qw:qwStrarray){
                ...
                if(rs){
                    if(qw.contains("恋爱")){
                        ...
                        TimePro.getDate(qw, qllagxsj, JBSS.datePattern);
                        break;
                    }
        ...
        if(qllagxsj.size()>0){
            qdlagxsj = (String) qllagxsj.get(0);
        }
     ...
     * 此方法把相识字符串里的所有时间都提取出来了，目前没有两个日期
     */
    public static  void JHDJSJ(Document document,Element newroot){
        Element root = document.getRootElement();
        Element JHDJSJNode = newroot.addElement("JHDJSJ").addAttribute("nameCN", "结婚登记时间");

        Pattern pattern = Pattern.compile(JBSS.datePattern);
        String jhdjsj = "未提及";
        ArrayList jhdjsjList = new ArrayList();
        if(root.attribute("value")!=null){
            String qwStr = root.attributeValue("value");
            String[] qwStrarray = qwStr.split("。|，|；");
            //结婚登记时间
            for(String qw:qwStrarray){
                ...
                if(rs){
                    if(qw.contains("结婚")){
                        System.out.println(qw);
                        TimePro.getDate(qw, jhdjsjList, JBSS.datePattern);
                        break;
                    }
        ...
        if(jhdjsjList.size()>0){
            jhdjsj =   (String) jhdjsjList.get(0);
        }
     ...
     * 4举办结婚仪式时间
     */
    public static  void JBJHYSSJ(Document document,Element newroot){
        Element root = document.getRootElement();
        Element JBJHYSSJNode = newroot.addElement("JBJHYSSJ").addAttribute("nameCN", "举办结婚仪式时间");
        Pattern pattern = Pattern.compile(JBSS.datePattern);
        String jbjhyssj = "未提及";
        ArrayList jbjhyssjList = new ArrayList();
        if(root.attribute("value")!=null){
            String qwStr = root.attributeValue("value");
            String[] qwStrarray = qwStr.split("。|，|；|,");
            //结婚登记时间
            for(String qw:qwStrarray){
                ...
                if(rs){
                    if((qw.contains("举行")||qw.contains("举办"))&&((qw.contains("结婚")&&(qw.contains("仪式")||qw.contains("典礼")))||qw.contains("婚礼"))){
                        System.out.println(qw);
                        TimePro.getDate(qw, jbjhyssjList, JBSS.datePattern);
                        break;
                    }
     ...
     * 时间点很不准确，很多没有时间
     */
    public static  void FQBH(Document document,Element newroot){
        Element root = document.getRootElement();
        ...
        ArrayList fqbhList = new ArrayList();  //夫妻不和开始时间
        ArrayList  fqbhyy = new ArrayList(); //夫妻不和原因
        if(root.attribute("value")!=null){
            String qwStr = root.attributeValue("value");
            String[] qwStrarray = qwStr.split("。");
            //夫妻不和的开始原因以及时间
                ...
                if(rs){
                    if(qw.contains("生活琐事")||qw.contains("矛盾")||qw.contains("性格不合")||
                        ...
                        String[] keyWords = {"生活琐事","矛盾","性格不合","吵架","感情破裂","感情已破裂","不睦","不和",
                                "争吵","殴打","虐待","暴力","吵打","打骂"};
                        fqbhList.add(TimePro.getRightDate(qw, keyWords, 0, JBSS.datePattern));
                        break;
                    }
        ...
        if(fqbhList.size()>0){
            fqbhkssj = (String) fqbhList.get(0);
        }
        if(fqbhyy.size()>0){
    ...
    public static void FJ(Document document,Element newroot){
        Element root = document.getRootElement();
        ...
        ArrayList ksfjsj = new ArrayList(); //开始分居时间
            ...
            for(String qw:qwStrarray){
                if(qw.contains("分居")&&(!qw.contains("从未分居")||!qw.contains("没有分居"))){
                    ...
                    if(rs){
                        //	    System.out.println("date"+qw);
                        String[] keyWords={"分居"};
                        ksfjsj.add(TimePro.getRightDate(qw,keyWords ,1,JBSS.datePattern));
                        break;
                    }
                }
            }
     ...
     * 7. 是否存在军婚情况及服役部队
     */
    public static void JHQKJFYBD(Document document,Element newroot){
        Element root = document.getRootElement();
