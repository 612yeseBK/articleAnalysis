    ...
    public static void main(String args[]){
        ...
        try{
            ...
            for(int j =0 ;j < list.length; j++){
                ...
                InfoSeek2.sfhyjbqx(document, ZYBZYLHQKNode);//17
                CZKZYHBZYJCHYQK.row18(document, ZYBZYLHQKNode);
                InfoSeek2.sfqpzj(document, ZYBZYLHQKNode); //19
                InfoSeek2.sfwtjwhh(document, ZYBZYLHQKNode);//20
                InfoSeek2.sfbbmbhy(document, ZYBZYLHQKNode);//21
                InfoSeek2.sfhybrq(document, ZYBZYLHQKNode);//22
                //23



                Element ZNFYQNode = newroot.addElement("ZVFYQGSXCMSS").addAttribute("nameCN", "子女抚养权归属需查明的事实");
                InfoSeek2.syznqk(document, ZNFYQNode);//24
                ZNFYQGS.row25(document, ZNFYQNode); //25
                InfoSeek2.zntsqk(document, ZNFYQNode);                                    //26
                ...
                InfoTest2.row29(document, LZSNNode); //29
                ...
                InfoTest2.row30(document, ZNFYQNode);//30
                //31



                Element CCFGXCMSSNode = newroot.addElement("CCFGXCMSS").addAttribute("nameCN", "财产分割需查明的事实");
                infoSeek.YGHQ(document, CCFGXCMSSNode);//32
                infoSeek.BGHQ(document, CCFGXCMSSNode);//33
                CCFGXCMSS.row34(document, CCFGXCMSSNode);
                infoSeek.YBGDWandSR(document, CCFGXCMSSNode);//35 //36
                //37



                Element FQGTZQZWQKNode = newroot.addElement("FQGTZQZWQK").addAttribute("nameCN", "夫妻共同债权、债务情况");
                infoSeek.FQZQ(document, FQGTZQZWQKNode);//38
                infoSeek.FQZW(document, FQGTZQZWQKNode);//39
                //40
